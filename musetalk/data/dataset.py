...
class FaceDataset(Dataset):
    ...
    def get_resized_mouth_mask(self, img_resized, landmark_array, face_shape, padding_pixel_mouth=0, image_size=256, crop_margin=0):
    ...
    def __getitem__(self, idx):
        ...
        while attempts < self.max_attempts:
            ...
            try:
                ...
                time.sleep(0.1)
                continue
            shift_landmarks, bbox_list_union, face_shapes = shift_landmarks_to_face_coordinates(landmark_list, bbox_list)
            ...
            for drive_idx in drive_idx_list:
                src_idx = get_src_idx(drive_idx, T, self.sample_method, shift_landmarks, face_shapes, self.top_k_ratio)
                if src_idx is None:
            ...
            for src_idx in src_idx_list:
                ...
                imSrc, _, _ = self.crop_resize_img(imSrc, bbox_s, self.crop_type, extra_margin=None)
            ...
            for drive_idx in drive_idx_list:
                ...
                imSameID, _ , mask_scaled_factor = self.crop_resize_img(imSameID, bbox_s, self.crop_type, extra_margin=extra_margin)
                ...
                face_mask = self.get_resized_mouth_mask(imSameID, shift_landmarks[drive_idx], face_shapes[drive_idx], self.padding_pixel_mouth, self.image_size, crop_margin=crop_margin)
            ...
            sample = dict(
                pixel_values_vid=torch.stack([self.to_tensor(imSameID) for imSameID in imSameIDs], dim=0),
                pixel_values_ref_img=torch.stack([self.to_tensor(ref_img) for ref_img in ref_imgs], dim=0),
                pixel_values_face_mask=torch.stack([self.pose_to_tensor(face_mask) for face_mask in face_masks], dim=0),
...
class HDTFDataset(FaceDataset):
    def __init__(self, cfg):
        ...
        list_paths = ['./dataset/HDTF/train.txt']
        repeats = [10]
...
class VFHQDataset(FaceDataset):
    def __init__(self, cfg):
        ...
        list_paths = ['./dataset/VFHQ/train.txt']
        repeats = [1]
        super().__init__(cfg, list_paths, root_path, repeats)
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        torchvision.utils.save_image(concatenated_img, f'debug/mask_check_{i}.jpg', nrow=2)
