...
class Net(nn.Module):
    ...
    def forward(self, input_latents, timesteps, audio_prompts):
        return self.unet(input_latents, timesteps, encoder_hidden_states=audio_prompts).sample

logger = logging.getLogger(__name__)

def initialize_models_and_optimizers(cfg, accelerator, weight_dtype):
    model_dict = {
        'vae': None, 'unet': None, 'net': None, 'wav2vec': None,
        'optimizer': None, 'lr_scheduler': None, 'scheduler_max_steps': None,
        'trainable_params': None
    }
    model_dict['vae'] = AutoencoderKL.from_pretrained(cfg.pretrained_model_name_or_path, subfolder=cfg.vae_type)
    unet_config_file = os.path.join(cfg.pretrained_model_name_or_path, cfg.unet_sub_folder + "/musetalk.json")
    ...
    model_dict['wav2vec'] = WhisperModel.from_pretrained(cfg.whisper_path).to(device="cuda", dtype=weight_dtype).eval()
    ...
    if cfg.solver.scale_lr:
        learning_rate = cfg.solver.learning_rate * cfg.solver.gradient_accumulation_steps * cfg.data.train_bs * accelerator.num_processes
    else:
...
def initialize_dataloaders(cfg):
    dataloader_dict = {'train_dataset': None, 'val_dataset': None, 'train_dataloader': None, 'val_dataloader': None}
    dataset_cfg = {
    ...
    dataloader_dict['train_dataset'] = PortraitDataset(cfg=dataset_cfg)
    dataloader_dict['train_dataloader'] = torch.utils.data.DataLoader(
        dataloader_dict['train_dataset'], batch_size=cfg.data.train_bs, shuffle=True, num_workers=cfg.data.num_workers)
    dataloader_dict['val_dataset'] = PortraitDataset(cfg=dataset_cfg)
    dataloader_dict['val_dataloader'] = torch.utils.data.DataLoader(
        dataloader_dict['val_dataset'], batch_size=cfg.data.train_bs, shuffle=True, num_workers=1)
...
def initialize_loss_functions(cfg, accelerator, scheduler_max_steps):
    ...
    if cfg.loss_params.gan_loss > 0:
        loss_dict['discriminator'] = MultiScaleDiscriminator(**cfg.model_params.discriminator_params).to(accelerator.device)
        ...
        loss_dict['scheduler_D'] = CosineAnnealingLR(loss_dict['optimizer_D'], T_max=scheduler_max_steps, eta_min=1e-6)

    if cfg.loss_params.mouth_gan_loss > 0:
        loss_dict['mouth_discriminator'] = MultiScaleDiscriminator(**cfg.model_params.discriminator_params).to(accelerator.device)
        ...
        loss_dict['mouth_optimizer_D'] = optim.AdamW(
            ...
            betas=cfg.discriminator_train_params.betas,
            eps=cfg.discriminator_train_params.eps)
        loss_dict['mouth_scheduler_D'] = CosineAnnealingLR(loss_dict['mouth_optimizer_D'], T_max=scheduler_max_steps, eta_min=1e-6)
    return loss_dict
...
def initialize_syncnet(cfg, accelerator, weight_dtype):
    if cfg.loss_params.sync_loss > 0 or cfg.use_adapted_weight:
        if cfg.data.n_sample_frames != 16:
            raise ValueError(f"Invalid n_sample_frames {cfg.data.n_sample_frames} for sync_loss, it should be 16.")
        syncnet_config = OmegaConf.load(cfg.syncnet_config_path)
        syncnet = SyncNet(OmegaConf.to_container(syncnet_config.model)).to(accelerator.device)
        print(f"Load SyncNet checkpoint from: {syncnet_config.ckpt.inference_ckpt_path}")
        checkpoint = torch.load(syncnet_config.ckpt.inference_ckpt_path, map_location=accelerator.device)
...
def initialize_vgg(cfg, accelerator):
    if cfg.loss_params.vgg_loss > 0:
        ...
        pyramid = vgg_face.ImagePyramide(cfg.loss_params.pyramid_scale, 3).to(accelerator.device)
        vgg_IN.eval()
        downsampler = Interpolate(size=(224, 224), mode='bilinear', align_corners=False).to(accelerator.device)
        return vgg_IN, pyramid, downsampler
    return None, None, None

def validation(cfg, val_dataloader, net, vae, wav2vec, accelerator, save_dir, global_step, weight_dtype, syncnet_score=1):
    net.eval()
    for batch in val_dataloader:
        ref_pixel_values = batch["pixel_values_ref_img"].to(weight_dtype).to(accelerator.device, non_blocking=True)
        pixel_values = batch["pixel_values_vid"].to(weight_dtype).to(accelerator.device, non_blocking=True)
        ...
        audio_prompts = rearrange(audio_prompts, '(b f) c h w -> (b f) (c h) w', b=bsz)
        image_pred_train = get_image_pred(pixel_values, ref_pixel_values, audio_prompts, vae, net, weight_dtype)
        image_pred_infer = get_image_pred(ref_pixel_values, ref_pixel_values, audio_prompts, vae, net, weight_dtype)
        process_and_save_images(
            batch, image_pred_train, image_pred_infer, save_dir, global_step,
            accelerator, cfg.num_images_to_keep, syncnet_score)
        break
    net.train()
