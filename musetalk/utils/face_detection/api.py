...
class FaceAlignment:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE, device='cuda', flip_input=False, face_detector='sfd', verbose=False):
        ...
        if 'cuda' in device:
            ...
            print('cuda start')
        module_name = 'musetalk.utils.face_detection.detection.' + face_detector
        face_detector_module = __import__(module_name, globals(), locals(), [face_detector], 0)
...
class YOLOv8_face:
    ...
    def post_process(self, preds, scale_h, scale_w, padh, padw):
        ...
        for pred in preds:
            ...
            bbox_pred = np.dot(self.softmax(tmp, axis=-1), self.project).reshape((-1,4))
            ...
            bboxes.append(bbox); scores.append(cls); landmarks.append(kpts)
        bboxes=np.concatenate(bboxes,axis=0); scores=np.concatenate(scores,axis=0); landmarks=np.concatenate(landmarks,axis=0)
        bboxes_wh=bboxes.copy(); bboxes_wh[:,2:4]=bboxes[:,2:4]-bboxes[:,0:2]
        classIds=np.argmax(scores,axis=1); confidences=np.max(scores,axis=1)
        mask=confidences>self.conf_threshold
        bboxes_wh=bboxes_wh[mask]; confidences=confidences[mask]; classIds=classIds[mask]; landmarks=landmarks[mask]
    ...
    def distance2bbox(self, points, distance, max_shape=None):
        x1=points[:,0]-distance[:,0]; y1=points[:,1]-distance[:,1]
        x2=points[:,0]+distance[:,2]; y2=points[:,1]+distance[:,3]
