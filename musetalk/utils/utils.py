...
def datagen(whisper_chunks, vae_encode_latents, batch_size=8, delay_frame=0, device="cuda:0"):
...
def get_image_pred(pixel_values, ref_pixel_values, audio_prompts, vae, net, weight_dtype):
    with torch.no_grad():
        ...
        input_latents = torch.cat([masked_latents, ref_latents], dim=1).to(weight_dtype)
...
def process_audio_features(cfg, batch, wav2vec, bsz, num_frames, weight_dtype):
    with torch.no_grad():
        ...
        audio_feats = torch.cat([
            torch.zeros_like(audio_feats[:, :2*cfg.data.audio_padding_length_left]),
            audio_feats,
...
def save_checkpoint(model, save_dir, ckpt_num, name="appearance_net", total_limit=None, logger=None):
    ...
    if total_limit is not None:
        checkpoints = [d for d in os.listdir(save_dir) if d.endswith(".pth") and name in d]
...
def delete_additional_ckpt(base_path, num_keep):
    dirs = [d for d in os.listdir(base_path) if d.startswith("checkpoint-")]
    num_tot = len(dirs)
    if num_tot <= num_keep:
        return
...
def process_and_save_images(batch, image_pred, image_pred_infer, save_dir, global_step, accelerator, num_images_to_keep=10, syncnet_score=1):
